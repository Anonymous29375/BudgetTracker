...
matplotlib.use(
    "Agg"
)  # Sets matplotlib to use a non-GUI backend so charts can be generated without a display
...
DATA_DIR = os.path.join(
    os.path.dirname(__file__), "transactions"
)  # Creates a path to the folder which stores CSV files
...
def list_csv_files():
    ...
    )  # List all files in the folder, keep only the .csv files, sort alphabetically, and return them
...
def load_csv(filename):
    ...
    ]  # Standardise the column names, lowercase and strip spaces

    missing = [
        c for c in EXPECTED_COLS if c not in df.columns
    ]  # Check if the required columns are missing
    if missing:
        abort(
            400, f"Missing columns: {missing}"
        )  # If the columns are missing, stop and return HTTP 400 error
    ...
    try:
        ...
        )  # Attempt conversion without dayfirst (in case of format errors)
    ...
    df_sorted = df.copy()
    df_sorted["date_sort"] = pd.to_datetime(
        df_sorted["date"], errors="coerce"
    )  # Temp column for sorting
    df_sorted = df_sorted.sort_values(by="date_sort", ascending=True).drop(
        columns=["date_sort"]
...
def classify_transactions(df):
    df["is_income"] = (
        df["amount"] > 0
    )  # Mark the transactions with positive amount as income
    df["is_expense"] = (
        df["amount"] < 0
    )  # Mark the transactions with negative amount as expense
    return df
...
def generate_chart(df):
    ...
    category_sums.plot(
        kind="bar", ax=ax, color=["green" if v > 0 else "red" for v in category_sums]
    )
    ...
    img = base64.b64encode(
        ...
        "utf-8"
    )  # Get the biniary data, convert to a base64 string, and convert from byte to regular string to be used in HTML
    plt.close(fig)
...
def report():
    ...
    if not filename:
        ...
        )  # If there is no file provided, return HTTP 400 error
    ...
    return render_template(
        ...
        table=df.to_dict(
            orient="records"
        ),  # Convert dataframe to list of dictionaries for table
